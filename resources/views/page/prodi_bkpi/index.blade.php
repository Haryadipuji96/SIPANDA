        ...
        <div class="p-6" x-data="{ openTambah: false, editId: null }">
            <div class="max-w-6xl mx-auto bg-white rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-2">📘 Dokumen MoU Prodi
                        BKPI</h1>
                ...
                <form id="bulkDeleteForm" action="{{ route('prodi_bkpi.bulkDelete') }}" method="POST">
                    @csrf
                    @method('DELETE')
                                ...
                                @forelse ($data as $index => $item)
                                    <tr id="row-{{ $item->id }}"
                                        ...
                                        (str_contains(strtolower($item->judul_mou), strtolower($highlight)) ||
                                            str_contains(strtolower($item->mitra_kerjasama ?? ''), strtolower($highlight)) ||
                                            str_contains(strtolower($item->nomor_dokumen ?? ''), strtolower($highlight)))
                                            ? 'fade-once'
                                            : '' }} ">
                                        ...
                                        <td class="border px-4 py-2 text-center">{{ $data->firstItem() + $index }}</td>
                                        ...
                                        <td class="border px-4 py-2 text-center">{{ $item->tanggal_berakhir ?? '-' }}
                                        </td>
                                        ...
                                        <td class="border px-4 py-2 text-center">
                                            <div class="flex items-center justify-center gap-3">
                                                ...
                                                <!-- Hapus -->
                                                <form action="{{ route('prodi_bkpi.destroy', $item) }}" method="POST"
                                                    class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                    ...
                                    <div x-show="editId === {{ $item->id }}" x-cloak
                                        ...
                                        <div class="bg-white p-6 rounded-lg w-96 shadow-lg">
                                            ...
                                            <form action="{{ route('prodi_bkpi.update', $item->id) }}" method="POST"
            ...
            <div x-show="openTambah" x-cloak
                ...
                <div class="bg-white p-6 rounded-lg w-96 shadow-lg">
                    ...
                    <form action="{{ route('prodi_bkpi.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
