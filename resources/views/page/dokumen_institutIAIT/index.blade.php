    ...
    <div class="p-4 md:p-6" x-data="{ openTambah: false, editId: null }">
        <div class="max-w-7xl mx-auto bg-white rounded-lg shadow-md p-4 md:p-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-3">
                <h1 class="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-2">📂 Dokumen MoU Institut
                    IAIT</h1>
                    ...
                    <tbody>
                        @forelse ($dokumen_iait as $index => $item)
                            <tr class="{{ $index % 2 === 0 ? 'bg-gray-200' : 'bg-gray-100' }}">
                                <td class="border px-4 py-2 text-center">{{ $dokumen_iait->firstItem() + $index }}</td>
                                ...
                                <td class="border px-4 py-2 text-center">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <!-- Hapus -->
                                        <form action="{{ route('dokumen_iait.destroy', $item) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                    ...
                                    <form action="{{ route('dokumen_iait.update', $item->id) }}" method="POST"
                                        ...
                                        <div class="space-y-2 text-sm">
                                            <input type="text" name="judul_mou" value="{{ $item->judul_mou }}"
                                                placeholder="Judul MoU" required
                                                class="w-full border px-2 py-1 rounded">

                                            <input type="text" name="nomor_mou" value="{{ $item->nomor_mou }}"
                                                placeholder="Nomor MoU" class="w-full border px-2 py-1 rounded">
                                            ...
                                            <input type="text" name="jenis_kerjasama"
                                                value="{{ $item->jenis_kerjasama }}" placeholder="Jenis Kerjasama"
                                                class="w-full border px-2 py-1 rounded">

                                            <input type="text" name="tingkat_kerjasama"
                                                value="{{ $item->tingkat_kerjasama }}" placeholder="Tingkat Kerjasama"
                                                class="w-full border px-2 py-1 rounded">

                                            <input type="date" name="tanggal_ttd" value="{{ $item->tanggal_ttd }}"
                                                class="w-full border px-2 py-1 rounded">
                                            ...
                                            <input type="text" name="penanggung_jawab"
                                                value="{{ $item->penanggung_jawab }}" placeholder="Penanggung Jawab"
                                                class="w-full border px-2 py-1 rounded">
            ...
            @include('components.pagination', ['data' => $dokumen_iait])
        </div>
