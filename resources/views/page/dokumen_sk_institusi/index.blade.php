    ...
    <div class="p-6" x-data="{ openTambah: false, editId: null }">
        <div class="max-w-6xl mx-auto bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-800">📂 Dokumen SK Institusi</h1>
            ...
            <form id="bulkDeleteForm" action="{{ route('dokumen_sk_institusi.bulkDelete') }}" method="POST">
                @csrf
                @method('DELETE')
                    ...
                    <table class="w-full text-sm border-collapse">
                        <thead class="bg-green-600 text-white">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">Unit Penerbit</th>
                        ...
                        <tbody>
                            @forelse ($dokumen_sk_institusi as $index => $item)
                                <tr id="row-{{ $item->id }}"
                                    class="
                                    {{ $index % 2 === 0 ? 'bg-gray-200' : 'bg-gray-100' }}
                                    {{ $highlight &&
                                    (str_contains(strtolower($item->judul), strtolower($highlight)) ||
                                        str_contains(strtolower($item->nomor_sk ?? ''), strtolower($highlight)) ||
                                        str_contains(strtolower($item->tanggal_sk ?? ''), strtolower($highlight)))
                                        ? 'fade-once'
                                        : '' }} ">
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                        {{ $dokumen_sk_institusi->firstItem() + $index }}</td>
                                    <td class="border px-4 py-2">{{ $item->judul }}</td>
                                    <td class="border px-4 py-2 text-center">{{ $item->nomor_sk ?? '-' }}</td>
                                    <td class="border px-4 py-2">{{ $item->tanggal_sk ?? '-' }}</td>
                                    <td class="border px-4 py-2">{{ $item->unit_penerbit ?? '-' }}</td>
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                        <div class="flex items-center justify-center gap-3">
                                            ...
                                            <!-- Hapus -->
                                            <form action="{{ route('dokumen_sk_institusi.destroy', $item) }}"
                                ...
                                <div x-show="editId === {{ $item->id }}" x-cloak
                                    ...
                                    <div class="bg-white p-6 rounded-lg w-96 shadow-lg">
                                        ...
                                        <form action="{{ route('dokumen_sk_institusi.update', $item->id) }}"
                                            ...
                                            <input type="text" name="nomor_sk" value="{{ $item->nomor_sk }}"
                                                class="w-full border px-2 py-1 mb-2 rounded text-sm">

                                            <input type="date" name="tanggal_sk" value="{{ $item->tanggal_sk }}"
                                                class="w-full border px-2 py-1 mb-2 rounded text-sm">

                                            <input type="text" name="unit_penerbit"
                                                value="{{ $item->unit_penerbit }}"
                                                class="w-full border px-2 py-1 mb-2 rounded text-sm">
                                            <textarea name="kategori" class="w-full border px-2 py-1 mb-2 rounded text-sm">{{ $item->kategori }}</textarea>
                ...
                @include('components.pagination', ['data' => $dokumen_sk_institusi])
        </div>
        ...
        <div x-show="openTambah" x-cloak
            ...
            <div class="bg-white p-6 rounded-lg w-96 shadow-lg">
                ...
                <form action="{{ route('dokumen_sk_institusi.store') }}" method="POST"
                    ...
                    <input type="text" name="nomor_sk" placeholder="Nomor SK"
                        class="w-full border px-2 py-1 mb-2 rounded text-sm">

                    <input type="date" name="tanggal_sk" placeholder="Tanggal SK"
                        class="w-full border px-2 py-1 mb-2 rounded text-sm">

                    <input type="text" name="unit_penerbit" placeholder="Unit Penerbit"
                        class="w-full border px-2 py-1 mb-2 rounded text-sm">
                    <textarea name="kategori" placeholder="Kategori" class="w-full border px-2 py-1 mb-2 rounded text-sm"></textarea>
