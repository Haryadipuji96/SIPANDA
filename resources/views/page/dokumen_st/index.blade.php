<x-app-layout>
    ...
    <div class="p-6" x-data="{ openTambah: false, editId: null }">
        <div class="max-w-6xl mx-auto bg-white rounded-lg shadow-md p-6">
            ...
            <form id="bulkDeleteForm" action="{{ route('dokumen_st.bulkDelete') }}" method="POST">
                @csrf
                @method('DELETE')
                            ...
                            @forelse ($dokumen_st as $index => $item)
                                <tr id="row-{{ $item->id }}"
                                    ...
                                    (str_contains(strtolower($item->judul), strtolower($highlight)) ||
                                        str_contains(strtolower($item->nomor_st ?? ''), strtolower($highlight)) ||
                                        str_contains(strtolower($item->tanggal_st ?? ''), strtolower($highlight)))
                                        ? 'fade-once'
                                        : '' }} ">
                                        @canAdmin
                                    ...
                                    @endcanAdmin
                                    <td class="border px-4 py-2 text-center">{{ $dokumen_st->firstItem() + $index }}
                                    </td>
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                        ...
                                        <div class="flex items-center justify-center gap-3">
                                            ...
                                            <!-- Hapus -->
                                            <form action="{{ route('dokumen_st.destroy', $item) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                ...
                @include('components.pagination', ['data' => $dokumen_st])
        </div>
