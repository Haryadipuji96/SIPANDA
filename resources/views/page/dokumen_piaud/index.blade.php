<x-app-layout>
    ...
    <div class="p-6" x-data="{ openTambah: false, editId: null }">
        <div class="max-w-6xl mx-auto bg-white rounded-lg shadow-md p-6">
            ...
            <form id="bulkDeleteForm" action="{{ route('dokumen_piaud.bulkDelete') }}" method="POST">
                @csrf
                @method('DELETE')
                @canAdmin
                ...
                <div class="table-wrapper border border-gray-200 rounded-lg">
                    <table class="w-full text-sm border-collapse">
                        ...
                        <tbody>
                            @forelse ($dokumen_piaud as $index => $item)
                                <tr id="row-{{ $item->id }}"
                                    class="
                                ...
                                (str_contains(strtolower($item->judul_dokumen), strtolower($highlight)) ||
                                    ...
                                    @endcanAdmin
                                    <td class="border px-4 py-2 text-center">{{ $dokumen_piaud->firstItem() + $index }}
                                    </td>
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                        ...
                                        <div class="flex items-center justify-center gap-3">
                                            ...
                                            <!-- Hapus -->
                                            <form action="{{ route('dokumen_piaud.destroy', $item) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                ...
                                <div x-show="editId === {{ $item->id }}" x-cloak
                                    ...
                                    <div class="bg-white p-6 rounded-lg w-96 shadow-lg">
                                        ...
                                        <form action="{{ route('dokumen_piaud.update', $item->id) }}" method="POST"
                ...
                @include('components.pagination', ['data' => $dokumen_piaud])
        </div>
        ...
        <div x-show="openTambah" x-cloak
            ...
            <div class="bg-white p-6 rounded-lg w-96 shadow-lg">
                ...
                <form action="{{ route('dokumen_piaud.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
