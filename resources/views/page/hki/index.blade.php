<x-app-layout>
    ...
    <div class="p-6" x-data="{ openTambah: false, editId: null }">
        <div class="max-w-6xl mx-auto bg-white rounded-lg shadow-md p-6">
            ...
            <form id="bulkDeleteForm" action="{{ route('hki.bulkDelete') }}" method="POST">
                @csrf
                @method('DELETE')
                            ...
                            @forelse ($hki as $index => $item)
                                <tr id="row-{{ $item->id }}"
                                    class="
                                        ...
                                        {{ $highlight &&
                                        (str_contains(strtolower($item->nama_dokumen_kegiatan), strtolower($highlight)) ||
                                            str_contains(strtolower($item->tahun ?? ''), strtolower($highlight)) ||
                                            str_contains(strtolower($item->fakultas ?? ''), strtolower($highlight)))
                                            ? 'fade-once'
                                            : '' }}
                                    ">
                                    ...
                                    <td class="border px-4 py-2 text-center">{{ $hki->firstItem() + $index }}</td>
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                        <div class="flex items-center justify-center gap-3">
                                            ...
                                            <!-- Hapus -->
                                            <form action="{{ route('hki.destroy', $item) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                ...
                @include('components.pagination', ['data' => $hki])
        </div>
